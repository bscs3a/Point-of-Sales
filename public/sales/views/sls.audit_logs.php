        ...
        <div class="container mx-auto px-4 sm:px-8">
            <div class="py-8">
                    ...
                    <div class="flex flex-row mb-1 sm:mb-0">
                        <div class="relative">
                            <form action="/auditlogSearch" method="post" class="flex">
                                <input type="text" id="search" placeholder="Search.." name="searchQueryAudit" value="<?php echo isset($_SESSION['postdata']['searchQueryAudit']) ? $_SESSION['postdata']['searchQueryAudit'] : ''; ?>" class="shadow appearance-none border rounded-l-lg w-full text-gray-700 leading-tight focus:outline-none focus:shadow-outline p-2">
                                <input type="hidden" name="pageNumber" value="<?php echo isset($_GET['page']) ? $_GET['page'] : 1 ?>">
                                <button type="submit" class="px-4 py-2 text-sm/none bg-gray-200 hover:bg-gray-300 text-gray-900 rounded-r-lg">
                                    <!-- Uploaded to: SVG Repo, www.svgrepo.com, Generator: SVG Repo Mixer Tools -->
                                    <svg fill="#000000" height="15px" width="15px" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 488.4 488.4" xml:space="preserve">
                                        <g>
                                            <g>
                ...
                <div class="overflow-x-auto mt-6">
                    <table id="auditLogsTable" class="table-auto w-full mx-auto rounded-lg overflow-hidden shadow-lg text-center">
                        <thead class="bg-gray-200">
                            <tr>
                                <th class="px-4 py-2 font-semibold">Audit ID</th>
                        ...
                        <tbody>
                            ...
                            if ($stmt->rowCount() > 0) {
                                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                    ...
                                    echo "<td class='px-4 py-2'>{$row['fullname']}</td>";
                                    echo "<td class='px-4 py-2'>{$row['datetime']}</td>";
                                    echo "<td class='px-4 py-2'>{$row['action']}</td>";
                                    echo "</tr>";
                                }
                            }
                ...
                <ol class="flex justify-end mr-8 gap-1 text-xs font-medium mt-5">
                    ...
                    <?php if ($page > 1) : ?>
                        <li>
                            <!-- CHANGE THE ROUTE -->
                            <a route="<?php echo $link . $page - 1 ?>" class="inline-flex size-8 items-center justify-center rounded border border-gray-100 bg-white text-gray-900 rtl:rotate-180">
                    ...
                    ?>
                        <li>
                            <a route="<?php echo $link . $i ?>" class="block size-8 rounded border <?= $i == $page ? 'border-blue-600 bg-blue-600 text-white' : 'border-gray-100 bg-white text-gray-900' ?> text-center leading-8">
                                <?= $i ?>
                            </a>
                        </li>
                    <?php endfor; ?>
